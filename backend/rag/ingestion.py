...
def process_document(
    ...
    load_document now automatically falls back to Groq vision-model OCR
    for image-based / scanned PDFs, so this function's logic is unchanged
    but now succeeds on documents that previously returned no content.
    """
    try:
        # Step 1: Load file content using lightweight parsers (PyMuPDF / Docx2txt),
        # with Groq OCR fallback for image-based PDFs handled inside load_document
